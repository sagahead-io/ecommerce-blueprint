...
type RabbitMqTransportConfigurationExtended = RabbitMqTransportConfiguration & {
...
export class RabbitMqTransport implements Transport<RabbitMqMessage> {
  ...
  private configuration!: RabbitMqTransportConfigurationExtended
  ...
  constructor(
    ...
    private readonly config: RabbitMqTransportConfiguration,
    ...
    this.configuration = {
      ...this.config,
      deadLetterExchange: this.config.deadLetterExchange || deadLetterExchange,
      deadLetterQueue: this.config.deadLetterQueue || deadLetterQueue,
    }
    this.maxRetries = this.configuration.maxRetries || DEFAULT_MAX_RETRIES
  }
  ...
  async fail(transportMessage: TransportMessage<unknown>): Promise<void> {
    ...
    this.channel.sendToQueue(this.configuration.deadLetterQueue, Buffer.from(serializedPayload), rawMessage.properties)
    this.logger.debug('Message failed immediately to dead letter queue', {
      rawMessage,
      deadLetterQueue: this.configuration.deadLetterQueue,
    })
  }
  ...
  async readNextMessage(): Promise<TransportMessage<RabbitMqMessage> | undefined> {
    ...
    const attributes: MessageAttributes = {
      ...
      attributes:
        rabbitMessage.properties.headers && rabbitMessage.properties.headers.attributes
          ? (JSON.parse(rabbitMessage.properties.headers.attributes as string) as MessageAttributeMap)
          : {},
      stickyAttributes:
        rabbitMessage.properties.headers && rabbitMessage.properties.headers.stickyAttributes
  ...
  private async assertExchange(messageName: string): Promise<void> {
    if (!this.assertedExchanges[messageName]) {
      ...
      await this.channel.assertExchange(messageName, this.configuration.exchangeType || 'fanout', { durable: true })
  ...
  private async bindExchangesToQueue(): Promise<void> {
    ...
    await this.channel.assertQueue(this.configuration.queueName, {
      durable: true,
      deadLetterExchange: this.configuration.deadLetterExchange,
    })
    const subscriptionPromises = this.handlerRegistry.messageSubscriptions.map(async (subscription) => {
      ...
      await this.channel.bindQueue(this.configuration.queueName, exchangeName, routeKey)
    })
  ...
  private async createDeadLetterQueue(): Promise<void> {
    await this.channel.assertExchange(this.configuration.deadLetterExchange, 'direct', { durable: true })
    await this.channel.assertQueue(this.configuration.deadLetterQueue, { durable: true })
    await this.channel.bindQueue(this.configuration.deadLetterQueue, this.configuration.deadLetterExchange, '')
  }
